...
void userHandle(double x, double y, double z, double t,
		double e, double dx, double dy, double dz,
							bool hole) {
  double um = 1e-04;
	if(z< -50*um && z> -50*um-5*um)energyatend->Fill(e);
	else if(z< -50*um-5*um)pofe->Fill(z);
...
int main(int argc, char * argv[]) {
 ...
 // plottingEngine.SetDefaultStyle();
  ...
  double pascal = 0.00750061683; //torr
//  const double pressure = 1 * AtmosphericPressure;
  const double pressure = 1.e-19 * pascal;
...
//  fm->PrintMaterials();
  ...
  TGeoVolume *world = mgr->MakeBox("world", medVacuum, 50*um, 50*um, 60*um);
  ...
  // comment out world volume to make the world vacuum
...
//  gas->WriteGasFile("ar_93_co2_7.gas");
 ...
 char hname[3];
 sprintf(hname, "energy%f.root",e0);
 energyatend->SaveAs(hname);
 sprintf(hname, "pofe%f.root",e0);
 pofe->SaveAs(hname);
 sprintf(hname, "initenergy%f.root",e0);
 initE->SaveAs(hname);
 sprintf(hname, "finalenergy%f.root",e0);
 finalE->SaveAs(hname);
